...
| `08_thousands_currency.csv` | `$1,234.56` 等 | W008 | `parseCurrency` (下記) |
| `09_quotes_commas.csv` | RFC 4180 quote escape | (none) | 正常処理 |
| `10_bom.csv` | UTF-8 BOM | (none) | 自動 strip |
| `11_semicolon_eu.csv` | `;` 区切り EU | W005 | sniff で自動修復 |
| `12_real.tsv` | TSV (拡張子正しい) | (none) | — |
| `13_crlf.csv` | tab + `.csv` 拡張子 + CRLF | W005 | 拡張子修正 or sniff |
| `14_wrong_ext.csv` | 同上 | W005 | 同上 |
| `15_trailing_blank.csv` | 末尾空行 | (none) | 自動 |
| `16_dates_units.csv` | `5kg` / `11.5cm` | W007 | `stripUnits` (下記) |
...
## Wide → Long 変形 (`hanalyze melt`)

「1 行 1 水準・列名が水準 (時刻 / 位置 / index 等)・歯抜けセル」の wide CSV を
**long-form (tidy)** に展開する。列名がそのまま新しい variable 列の値になり、
NA セルは行ごと自動 drop。
