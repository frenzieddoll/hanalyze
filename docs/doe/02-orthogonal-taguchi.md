...
`Hanalyze.Design.Orthogonal.mkL2k k` performs this. First four rows of L8 (k=3):
...
L9, L12, L18 cannot be generated by simple subsets, so `Hanalyze.Design.Orthogonal` hard-codes the
Taguchi standard tables.

---

## 4. Using `Hanalyze.Design.Orthogonal`
...
## 6. Using `Hanalyze.Design.Taguchi`
...
`OAMetadata` (`Hanalyze.Design.Orthogonal.listArraysWithSize`) returns structured
...
## 8. HTML reports (`Hanalyze.Viz.Taguchi`)
...
### Building the report through `Hanalyze.Viz.ReportBuilder`
...
Use `Hanalyze.Viz.Taguchi` for convenience and `Hanalyze.Viz.ReportBuilder` for flexibility.
...
| ≤ 13 (3-level) | L27(3¹³) (not implemented in `Hanalyze.Design.Orthogonal`) |
...
The standard tables are validated in `Hanalyze.Design.Orthogonal`'s tests for **column balance**
...
- [../visualization/02-report-builder.md](../visualization/02-report-builder.md) — custom reports via `Hanalyze.Viz.ReportBuilder`
