...
作例は [io/04-fit-api](../io/04-fit-api.md) ・ [io/01-dirty-data](../io/01-dirty-data.md) を参照。
...
正本は [io/04-fit-api](../io/04-fit-api.md) ([12 plot](12-plot.md) で描画へ)。
...
→ R4DS Ch13 の定番。 [io/04-fit-api](../io/04-fit-api.md) 等の作例で使用。
...
集約子 (`meanOf` / `quantileOf` / `nOf` / `nDistinctOf` 等) は `Stat.Descriptive` に委譲。 → [Stat](10-stat.md)
