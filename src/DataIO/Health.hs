...
import DataIO.Log (LogEntry, LogReport, mkWarn, logReport, noLog)
import DataIO.Convert (getMaybeTextVec)
import DataIO.Preprocess (isNAString)
...
detectDuplicateBlankNames df =
  ...
  in mconcat
       ...
       , if null trimmedDiffer then noLog
                  ...
                  (Just "DataIO.Preprocess.renameColumn でリネームできます。")
       , if null dups then noLog
           else mk "W004"
                  ("重複した列名: "
                     <> T.intercalate ", " dups
        ...
        in if Map.size naSet >= 2
             then logReport
                    (mkWarn "W006"
                       ...
                       (Just "DataIO.Preprocess.imputeMean / dropMissingRows で正規化できます。"))
             else noLog
