"""hmm: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
def main():
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["mu_1"])
        ess = float(s.loc["mu_1", "ess_bulk"])
        rhat = float(s.loc["mu_1", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(mu_1)={ess:7.1f}  "
