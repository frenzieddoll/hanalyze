"""earnings-log10earn_height (posteriordb) — PyMC 実装。
log10(earn) ~ height の線形回帰。★2026-07-13: コード準備のみ (未実行)。
"""
import json
...
def log10earn_height_model():
    d = read_data()
    log10_earn = np.log10(d["earn"])
    with pm.Model() as m:
        beta1 = pm.Normal("beta1", mu=0.0, sigma=1000.0)
        beta2 = pm.Normal("beta2", mu=0.0, sigma=1000.0)
        sigma = pm.HalfCauchy("sigma", beta=25.0)
        pm.Normal("log10_earn", mu=beta1 + beta2 * d["height"], sigma=sigma, observed=log10_earn)
    return m


def main():
    m = log10earn_height_model()
    with m:
