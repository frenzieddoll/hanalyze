"""low-dim-gauss-mix: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import low_dim_gauss_mix_model  # noqa: E402
...
def main():
    for label, kwargs in combos:
        ...
        ess = float(s.loc["theta", "ess_bulk"])
        rhat = float(s.loc["theta", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(theta)={ess:7.1f}  "
