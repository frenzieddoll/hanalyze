"""seeds: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import seeds_model  # noqa: E402
...
def main():
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["alpha0"])
        ess = float(s.loc["alpha0", "ess_bulk"])
        rhat = float(s.loc["alpha0", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(alpha0)={ess:7.1f}  "
