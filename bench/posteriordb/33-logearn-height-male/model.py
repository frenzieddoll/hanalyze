"""earnings-logearn_height_male (posteriordb) — PyMC 実装。
log(earn) ~ height + male の線形回帰。★2026-07-13: コード準備のみ (未実行)。
"""
import json
...
def logearn_height_male_model():
    ...
    with pm.Model() as m:
        ...
        mu = beta1 + beta2 * d["height"] + beta3 * d["male"]
        pm.Normal("log_earn", mu=mu, sigma=sigma, observed=log_earn)
    return m


def main():
    m = logearn_height_male_model()
    with m:
