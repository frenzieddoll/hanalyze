...
-- | earnings-logearn_height_male (posteriordb) — hanalyze (ModelP) 実装。
-- log(earn) ~ height + male の線形回帰。
...
dataPath :: FilePath
dataPath = "bench/posteriordb/33-logearn-height-male/data/earnings.json"

figuresDir :: FilePath
figuresDir = "bench/posteriordb/33-logearn-height-male/figures"

readData :: IO EData
readData = either fail pure =<< eitherDecodeFileStrict dataPath

logEarnHeightMaleModel :: ModelP ()
logEarnHeightMaleModel = do
  ...
  plateForM_ "obs" (zip3 heights males logEarn) $ \(h, mm, le) ->
    observe "log_earn" (Normal (beta1 + beta2 * h + beta3 * mm) sigma) [le]
...
main = do
  ...
  let logEarn = map log (eEarn d)
      ...
      m = df |-> hbm cfg logEarnHeightMaleModel

  putStrLn $ "synthVecIR = " ++
    (case synthVecIR logEarnHeightMaleModel of
