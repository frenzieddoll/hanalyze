"""lsat: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import lsat_model  # noqa: E402
...
def main():
    for label, kwargs in combos:
        ...
        ess = float(s.loc["beta", "ess_bulk"])
        rhat = float(s.loc["beta", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(beta)={ess:7.1f}  "
