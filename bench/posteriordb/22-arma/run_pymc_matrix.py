"""arma: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import arma_model  # noqa: E402
...
def main():
    ...
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["phi"])
        ess = float(s.loc["phi", "ess_bulk"])
        rhat = float(s.loc["phi", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(phi)={ess:7.1f}  "
