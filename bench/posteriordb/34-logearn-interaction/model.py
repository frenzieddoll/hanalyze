"""earnings-logearn_interaction (posteriordb) — PyMC 実装。
...
def logearn_interaction_model():
    d = read_data()
    log_earn = np.log(d["earn"])
    inter = d["height"] * d["male"]
    with pm.Model() as m:
        ...
        mu = beta1 + beta2 * d["height"] + beta3 * d["male"] + beta4 * inter
        pm.Normal("log_earn", mu=mu, sigma=sigma, observed=log_earn)
    return m


def main():
    m = logearn_interaction_model()
    with m:
