"""bones: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import bones_model  # noqa: E402
...
def main():
    # Phase 101 A1: argv でラベル絞り込み (無指定 = 全 combo)
    wanted = sys.argv[1:]
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["theta"])
        ess = float(s.loc["theta[0]", "ess_bulk"])
        rhat = float(s.loc["theta[0]", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(theta[0])={ess:7.1f}  "
