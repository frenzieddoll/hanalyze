"""TODO(<model_name>): PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
00-template は雛形 (ダミー値・NN-<slug>/ にコピーして使う)。実例は
../01-glm-poisson/run_pymc_matrix.py を参照。
"""
import os
import sys
import time
...
from model import template_model  # noqa: E402
...
def main():
    for label, kwargs in combos:
        ...
        # TODO: var_names をこのモデルの主要パラメータに置き換える。
        s = az.summary(idata, var_names=["alpha"])
        ess = float(s.loc["alpha", "ess_bulk"])
        rhat = float(s.loc["alpha", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(alpha)={ess:7.1f}  "
