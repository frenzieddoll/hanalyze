"""radon: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import radon_model  # noqa: E402
...
def main():
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["mu_alpha"])
        ess = float(s.loc["mu_alpha", "ess_bulk"])
        rhat = float(s.loc["mu_alpha", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(mu_alpha)={ess:7.1f}  "
