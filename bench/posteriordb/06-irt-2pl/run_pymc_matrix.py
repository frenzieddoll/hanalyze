"""irt-2pl: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import irt_2pl_model  # noqa: E402
...
def main():
    # Phase 105 A1: argv でラベル絞り込み (無指定 = 全 combo・Phase 101 A1 と同型)
    wanted = sys.argv[1:]
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["mu_b"])
        ess = float(s.loc["mu_b", "ess_bulk"])
        rhat = float(s.loc["mu_b", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(mu_b)={ess:7.1f}  "
