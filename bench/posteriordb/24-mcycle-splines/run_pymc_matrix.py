"""mcycle-splines: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import spline_model  # noqa: E402
...
def main():
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["Intercept"])
        ess = float(s.loc["Intercept", "ess_bulk"])
        rhat = float(s.loc["Intercept", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(Intercept)={ess:7.1f}  "
