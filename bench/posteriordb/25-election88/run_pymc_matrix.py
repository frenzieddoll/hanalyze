"""election88: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import election_model  # noqa: E402
...
def main():
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["beta1"])
        ess = float(s.loc["beta1", "ess_bulk"])
        rhat = float(s.loc["beta1", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(beta1)={ess:7.1f}  "
