"""eight-schools: PyMC の CPU 限定 sampler×backend マトリクスを回し、最速を選ぶ。
...
from model import eight_schools_model  # noqa: E402
...
def main():
    for label, kwargs in combos:
        ...
        s = az.summary(idata, var_names=["mu"])
        ess = float(s.loc["mu", "ess_bulk"])
        rhat = float(s.loc["mu", "r_hat"])
        print(f"{label:14s} wall={ms:9.1f}ms  ess_bulk(mu)={ess:7.1f}  "
