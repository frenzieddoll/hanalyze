...
最終更新: 2026-05-07 (P1-P6 perf 改善 + B7-B13 全 Tier 拡充後)
...
| Lasso_n1000_p5 | **0.076** | 0.700 | **9.22×** | 0.7696 | 0.7696 | 0.7696 ✅ | small-prob |
| **Lasso_n10000_p50** | 6.16 | **2.82** | 0.46× | 0.7644 | 0.7644 | 0.7644 ✅ | sklearn Cython CD |
| EN_n1000_p5 | **0.078** | 0.500 | **6.40×** | 0.7622 | 0.7622 | 0.7622 ✅ | small-prob |
| **EN_n10000_p50** | 6.41 | **2.86** | 0.45× | 0.7568 | 0.7568 | 0.7568 ✅ | sklearn Cython CD |
...
| GLM_poisson_n10000_p20 | **2.42** | 3.70 | **1.53×** | 0.369 | 0.410 | (0.4+) ≈ | |
...
**Python 勝ち**: Lasso / EN @ n=10k (sklearn Cython CD inner loop, FFI 領域)。
...
| NW_n1000_p5 | 45.5 | **9.43** | 0.21× | 0.905 | 0.905 | 0.905 ✅ | |
...
|---|---:|---:|---:|---:|---:|---:|---|
| ZDT1/NSGA-II | 634.7 | **453.8** | 0.71× | 0.870 | 0.839 | ~0.84 | hanalyze HV 勝ち |
| ZDT2/NSGA-II | 680.0 | **484.6** | 0.71× | 0.471 | 0.484 | ~0.48 | pymoo HV 微勝 |
| ZDT3/NSGA-II | 652.6 | **447.0** | 0.69× | 1.328 | 1.291 | ~1.29 | hanalyze HV 勝ち |
| DTLZ2_3/NSGA-II | 540.7 | **440.6** | 0.81× | 2.747 | 2.722 | ~2.72 | hanalyze HV 勝ち |

**Python 勝ち**: 全 4 ベンチで **時間** (per-gen 速度差、~25%); hanalyze の HV は同等以上。
NF1+NF3+NF4 phase で pymoo 並み quality に到達済 (時間は内部 Matrix 化で更に縮められる候補)。
...
| KMeans_n2000_p5_k5 | 413.9 | **23.3** | 0.06× ⚠ | 18458 | 18496 | inertia ≈ (sklearn k-means++ 高速) |
...
**Python 勝ち**: KMeans / DT (sklearn Cython tree split、k-means++)。
...
| WAIC_S1000_N200 | 14.5 | **6.29** | 0.43× | 417.0 | 417.0 | 完全一致 ✅ |
| **LOO_PSIS_S1000_N200** | **22.4** | 38.8 | **1.73×** | 417.0 | 417.0 | 完全一致 ✅ |
...
| Quantile_n10k_p20_tau0.5 | 362.7 | **198.6** | 0.55× | — | — | (interior-point 共通) |
| GAM_n2000_p2_d3_k5 | 14.4 | **9.82** | 0.68× | — | — | (両側 fit のみ) |
| **Spline_PCHIP_n1000** | **0.20** | 0.236 | **1.19×** ≈ | sum=3338 | sum=3338 | 完全一致 ✅ |

**Python 勝ち**: Quantile (interior-point の C 実装)、GAM (pygam Cython)。
...
| **BH_pAdjust_n1000** | **0.086** | 0.042 | **0.49×** | n_sig=0 | n_sig=0 | 同 ✅ (P2 で 2.20→0.086 ms = 27× 改善) |
| Bootstrap_mean_n1000_B1000 | 22.6 | **11.7** | 0.52× | CI幅=0.090 | 0.099 | 同 ✅ |
| AUC_LogLoss_n10000 | 5.60 | **4.37** | 0.78× | AUC=1.0 | AUC=1.0 | 完全一致 ✅ |
| Halton_n10000_d5 | 3.57 | **1.12** | 0.31× | n=10000 | n=10000 | (両側同じ low-discrepancy) |

**Python 勝ち**: Bootstrap, BH (statsmodels Cython), Halton (scipy.qmc C), AUC (sklearn Cython)。
...
| regression | Lasso_n10000_p50 | 0.46× | sklearn Cython coordinate descent |
| regression | EN_n10000_p50 | 0.45× | 同上 |
| ml | KMeans_n2000_p5_k5 | 0.06× ⚠ | sklearn k-means++ + Cython |
...
| mcmc_extras | WAIC | 0.43× | arviz Cython compute |
...
| stat_util | AUC_LogLoss | 0.78× | sklearn Cython sort + integrate |
...
| survts | Quantile | 0.55× | statsmodels interior-point C |
...
pure Haskell + hmatrix の枠で出来る改善は P1-P6 で到達。残ギャップは以下の領域:
