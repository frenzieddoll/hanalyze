...
通貨や単位混入などを **警告コード (W001〜W008) で自動検出** し、ロード時の
delimiter / コメント / ヘッダ有無は冒頭バイト列から自動推論する。残った
スケール変換は **クリーニング DSL** で列ごとに正規化:
```
...
hanalyze clean data.csv --rule price=CoerceNumeric   # 万能変換 (上記を順に試行)
```
CLI 全サブコマンドで `--strict` (警告で停止 / CI 用)、`--skip N`、
`--comment CH`、`--no-header` 等が共通で使える。詳細:
