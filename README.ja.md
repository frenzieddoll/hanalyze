...
| ページ | 内容 |
|---|---|
| [汚いデータの読込ガイド](docs/io/01-dirty-data.ja.md) | W001..W008 / LoadOpts / 19 fixture / CLI 修復例 (Phase A) |
...
| `hbm-regression`  | ベイズ単回帰 + ReportBuilder (DAG・MCMC・信用区間) | HBM 回帰のレポート統合 |
...
# ベイズ線形回帰 (HBM): NUTS で α/β/σ の事後を推定 → HTML レポート
...
```
DataIO/
  CSV.hs           -- 標準・防衛的ローダ (loadAuto / loadAutoSafe / loadAutoSafeWith)
                   -- LoadOpts (--no-header / --skip / --comment / --strict) 対応
  External.hs      -- Hackage dataframe を活用した Parquet / JSON ローダ
  Convert.hs       -- DataFrame から V.Vector Double / Text を取り出す共通ヘルパ
                   -- (deepseq で Hackage の例外を確実に捕捉)
  Preprocess.hs    -- 欠損補完 / フィルタ / 派生列 / 列選択 / groupBy
  Log.hs           -- 構造化ログ (Severity / LogEntry / LogReport / Loaded)
  Health.hs        -- 健全性検査 (W001 ヘッダ無し ... W008 通貨・桁区切り、計 9 種)
...
Viz/
  ...
  ReportBuilder.hs -- ★ コンポジション型レポート (ReportSection + Reportable typeclass、
                                                    対話的予測 + MCMC 診断対応、今後の標準)
  ReportInstances.hs -- Reportable RegFit/SplineFit/KernelRidgeFit/RFFRidgeFit/RobustGPFit
  AnalysisReport.hs -- 【非推奨】LM/GLM/GLMM/GP/HBM 専用 sum-type ベース (CLI regress --report 互換のため残置、将来削除)
