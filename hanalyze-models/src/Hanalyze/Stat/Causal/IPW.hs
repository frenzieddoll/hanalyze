...
--
-- [日本語]: Inverse Probability Weighting (IPW) による ATE / ATT 推定。
--
...
-- @
--
-- [English]: ATE\/ATT estimation via Inverse Probability Weighting (IPW).
--
-- Hajek-normalized estimator (stable in finite samples, lower variance
-- than Horvitz-Thompson):
--
-- @
--   ATE = Σ(T·Y/p) / Σ(T/p)  -  Σ((1-T)·Y/(1-p)) / Σ((1-T)/(1-p))
--   ATT = Σ(T·Y) / Σ T       -  Σ((1-T)·(p/(1-p))·Y) / Σ((1-T)·(p/(1-p)))
-- @
--
-- Here @p_i@ is P(T=1 | X_i) estimated by 'PropensityScore'. The weights
-- are computed with hmatrix Vector operations via
-- @PropensityScore.ipwWeights@ \/ @attWeights@.
--
-- ## Usage
--
-- @
--   let r = ipw xConf treat outcome           -- estimates PS from covariates + trims internally
--   print (ipwATE r, ipwATT r)
--
--   -- If the PS is already computed \/ a custom trim is wanted:
--   let ps' = trimPropensity 0.05 0.95 (propensityScore x t)
--       r'  = ipwWith ps' t y
-- @
--
...
-- | [日本語]: 既定の PS trim 範囲 @(0.01, 0.99)@ (Rosenbaum 慣例)。
--   [English]: Default PS trim range @(0.01, 0.99)@ (Rosenbaum convention).
...
-- | [日本語]: 共変量 @X@、 二値処置 @T@、 結果 @Y@ から ATE / ATT を IPW で推定。
--   内部で 'propensityScore' + 'defaultPSTrim' を適用。
--   [English]: Estimate ATE\/ATT via IPW from covariates @X@, binary
--   treatment @T@, and outcome @Y@. Applies 'propensityScore' +
--   'defaultPSTrim' internally.
...
-- | [日本語]: 既に算出 (+trim) 済の PropensityScore を再利用する版。 同じ X から
--   ATE / ATT を複数バリアントで比べたい場合に有用。
--   [English]: A variant that reuses an already-computed (+trimmed)
--   PropensityScore. Useful for comparing ATE\/ATT across multiple
--   variants from the same X.
